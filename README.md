...
- The Babashka test suite passes: 273 tests, 1274 assertions, 0 failures, 0 errors.
- **The flake this file used to disclaim is fixed, and the fix came from upstream.** `pack-board-serializes-concurrent-audit-increments` failed here about one run in five, losing two of eight concurrent increments. `pack_board.bb` took the board lock *after* checking that the board file existed, so two writers could both pass the check and the second could drop the first's increment. Upstream's `d15af3d` moves the check inside the lock and makes the replace atomic. Measured after merging it: 20 consecutive runs of that test, 0 failures — at the old rate 20 runs would have failed about four times.
...
- **Upstream is merged to `d15af3d`, deliberately not further.** That brings the reverse-handoff hops (`forward-only`, `back-one`, `back-all` on a window line, and the rule that an inbound reverse tree is the structure to replay onto) and the board-lock fix above. The seven commits after it turn SwarmForge into a multi-project forge with a `lieutenant` chat role; they conflict in four files and are a feature this fork does not need yet, so they are left on upstream. Merging to `d15af3d` conflicted in `bb.edn` only, where both sides had added a test namespace to the same two lines.
- **`bb crap` and `bb coverage:bb` do not run from this checkout, and the reason is the folder name.** Upstream's `d15af3d` adds them; getting them to work here took two fixes and one refusal. `sfk/support.clj` declares `(ns sfk.support)` but lives in `swarmforge/scripts/kotlin/sfk/`, so from `swarmforge/scripts` that name resolves nowhere and Cloverage dies while ordering dependencies — before `--ns-exclude-regex` is consulted, which is why excluding it is not a workaround; `bb.edn` now puts that directory on `:paths`, the same classpath the Kotlin tools build for themselves at runtime. Past that, Cloverage builds a `java.net.URI` from each source path without encoding it and dies on the space in `AI Agents Workflow - Uncle Bob`, after the entire analysis has run. Measured both ways: the same tree copied to a space-free path completes and prints a table, `sfk.support` included. That is a Cloverage defect with nothing here to fix, so both tasks now refuse up front with the folder name and the reason instead of a stack trace forty lines deep. `bb test` is unaffected.
